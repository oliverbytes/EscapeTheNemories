...
public static class Uni2DUtilsMath
{
	...
	{
		return HorizontalFOVDeg( Camera.mainCamera );
	}
	...
	{
		return DistanceToCameraFromFrustumWidth( Camera.mainCamera, a_fFrustumWidth );
	}
	...
	{
		return DistanceToCameraFromFrustumHeight( Camera.mainCamera, a_fFrustumHeight );
	}
	...
	// Return true if a_f3Point is inside the triangle ABC (in 2D). Vertices A, B & C must be ordered CCW
	...
	// Return true if a 2D ray intersects a 2D segment defined by a_f2SegmentStart and a_f2SegmentEnd
	...
	public static bool Raycast2DSegment( Ray a_rRay, Vector2 a_f2SegmentStart, Vector2 a_f2SegmentEnd, out float a_fRayHitDistance )
	{
		...
		Vector2 f2PerpRay = Uni2DUtilsMath.PerpVector2( a_rRay.direction );

		// Vector orthogonal to segment vector
		Vector2 f2PerpSegment = Uni2DUtilsMath.PerpVector2( a_f2SegmentEnd - a_f2SegmentStart );
	...
	// Return an orthogonal Vector2 to the specified Vector2
	public static Vector2 PerpVector2( Vector2 a_f2Vector )
	{
		return new Vector2( a_f2Vector.y, - a_f2Vector.x );
	}

	// Return an orthogonal Vector2 to the specified Vector3 (z-value dismissed)
